...
public class GestorPrestamo {
	...
	public static ArrayList<Prestamo> selectPrestamosByEjemplar(String ejemplar) throws ClassNotFoundException, SQLException {
		...
		while(rs.next()){
			prestamos.add(new Prestamo(rs.getInt("id"),GestorEjemplar.selectEjemplarByCodigo(rs.getString("ejemplar")),GestorUsuario.selectUsuarioByDNI(rs.getString("usuario")),rs.getDate("fecha_ini"),rs.getDate("fecha_fin")));
	}
		return prestamos;	
	}
}
